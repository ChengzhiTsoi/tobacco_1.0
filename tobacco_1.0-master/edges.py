...
## This code will read a building block cif file for an edge. 
...
def __edge_properties(arg):
	...
	with open("edges_bb/"+arg, 'r') as edge_cif:
		edge = edge_cif.read()
	for line in edge.split("\n"):
	...
	ay = 0.0
	az = 0.0
	bx = length_b * math.cos(gamma * math.pi / 180.0)
	by = length_b * math.sin(gamma * math.pi / 180.0)
	bz = 0.0
	cx = length_c * math.cos(beta * math.pi / 180.0)
	cy = length_c * length_b * math.cos(alpha * math.pi /180.0) - bx * cx / by
...
##### TURN LISTS INTO ARRAYS AND BUILD MATRIX
	...
	### Distance from centroid to connection site
	...
	for pair in itertools.combinations(connection_site, 2):
		...
		angle = round(np.arccos(np.dot(*pair)/(np.linalg.norm(pair[0])*np.linalg.norm(pair[1])))*180/np.pi,2)

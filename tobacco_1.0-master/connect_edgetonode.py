...
#Connect edge to two nodes of the same type



def __edge_to_node(unit_cell, e1_coord, v11_nbors_for_e1, node_1_elements, edge_elements, node_1_frac_coord, edge_frac_coord, node_1_connectivity, edge_connectivity):
	elements=[]
	frac_coords=[]
	#Update indexes of atoms in structure, node first, then edge.  
	...
	for k in range(len(v11_nbors_for_e1)):
		node_1_connection = node_1_elements[v11_nbors_for_e1[k][0]] 
		node_2_connection = node_1_elements[v11_nbors_for_e1[k][1]]
		...
		for i in [i for i, x in enumerate(node_1_connection) if x[0][0][0] == 'X']:
			...
			if len(node_1_connection)==1:
				connection_frac_coord_1.append(node_1_frac_coord[v11_nbors_for_e1[k][0]])
			elif not len(node_1_connection)==1:
				connection_frac_coord_1.append(node_1_frac_coord[v11_nbors_for_e1[k][0]][i]) # grab connection coord of first node
			
		for i in [i for i, x in enumerate(node_2_connection) if x[0][0][0] == 'X']:
			location_2.append(i)
			if len(node_2_connection)==1:
				connection_frac_coord_2.append(node_1_frac_coord[v11_nbors_for_e1[k][1]])
			elif not len(node_2_connection)==1:
				connection_frac_coord_2.append(node_1_frac_coord[v11_nbors_for_e1[k][1]][i]) # grab connection coord of second node
		...
		if len(node_1_connection)==1:
			connection_node_1_coord = node_1_frac_coord[v11_nbors_for_e1[k][0]]
		elif not len(node_1_connection)==1:
			connection_node_1_coord = node_1_frac_coord[v11_nbors_for_e1[k][0]][location_1[index_of_connection_1]]
		if len(node_2_connection)==1:
			connection_node_2_coord = node_1_frac_coord[v11_nbors_for_e1[k][1]]
		elif not len(node_2_connection)==1:
			connection_node_2_coord = node_1_frac_coord[v11_nbors_for_e1[k][1]][location_2[index_of_connection_2]]
		...
		atom_index_conn_1 = index_conn_1 + len(edge_elements[0])*k + len(node_1_elements)*len(node_1_elements[0])
	
		index_conn_2 = location_1[index_of_connection_1] + 1
		atom_index_conn_2 = index_conn_2 + len(node_1_elements[0])*v11_nbors_for_e1[k][0]
		...
		atom_index_conn_2 = index_conn_2 + len(node_1_elements[0])*v11_nbors_for_e1[k][1]
		##Add connection to connectivity information, taking into account periodic boundary conditions
		if all(x==5 for x in symm):
